...
def naiveSoftmaxLossAndGradient(
    ...
    probs = softmax(outsideVectors @ centerWordVec)
    loss = -np.log(probs[outsideWordIdx])

    probs[outsideWordIdx] -= 1
    gradCenterVec = outsideVectors.T.dot(probs)
...
def negSamplingLossAndGradient(
    ...
    dots = outsideVectors[indices].dot(centerWordVec)
    dots[1:] = -dots[1:]
    loss = np.sum(-np.log(sigmoid(dots)))
    
    # Not Checked
    gradCenterVec = (
        -(1 - dots[0])*outsideVectors[outsideWordIdx] 
        + np.sum((1-dots[1:]).dot(outsideVectors[negSampleWordIndices]))
    )
    ### END YOUR CODE
